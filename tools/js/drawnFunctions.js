...
$(document).ready(function() {
	...
	$("#dataForm").on("submit", function(event) {
		...
		inputsImage.find("img").each(function(i, o) {
			$(o).attr("src", "http://www.simumath.net/tools/drawn/"+$(o).attr("src"));
		...
		.done(function(data) {
			...
			.done(function(data) {
				...
				//$("#drawnResults-"+time).dialog( "option", "title", $(data).find("span.data-title").html());
				//$("#drawnResults-"+time).find("span.data-title").remove();
...
function showResults(title, content, type, time) {
	if(type == "chart") {
		if($("#drawnChart-"+time).length > 0) {
			$("#drawnChart-"+time).dialog("close");
			$("#drawnChart-"+time).remove();
		}
	}
	
	var newDialog = document.createElement("div");
	$(newDialog).attr("id", type == "results" ? "drawnResults-"+time : "drawnChart-"+time);
	...
	if(type == "chart") {
		thisButtons.unshift({text: "Распечатать",
			...
			click: function() {
				...
				valuesArray = [];
				titlesArray = [];
				if($("div.drawnResults-"+time+" input.TAB"+tabNum+"_0:checked").length > 0) {
					$("div.drawnResults-"+time+" input.TAB"+tabNum+"_0:checked").each(function(i, o) {
...
function fillOutput(resultData, time) {
	...
	$("div.drawnResults-"+time+" div#resultsTab div.ui-widget-content").css("display", "inline-block");
			
	if(resultData[4]["KAV"] != "0") {
		if(typeof resultData[0] != 'undefined') {
			for(var i = 0; i < resultData[0].length; i++) {
				$("div.drawnResults-"+time+" div.KAV_0 span:eq("+i+")").html(repAll(resultData[0][i], "\n", "<br/>"));
			}
			$("div.drawnResults-"+time+" div.KAV_0").css("display", "block");
		}
	}
	else {
		$("div.drawnResults-"+time+" div.KAV_0").css("display", "none");
	}
	
	if(typeof resultData[1] != 'undefined') {
		var thisM = resultData[4]["M"];
		$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:not(:last)").remove();
		...
		for(var i = 0; i <= resultData[4]["NSW"]; i++) {
			$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last").clone().insertAfter("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last");
			for(var v = 0; v < cellsCount; v++) {
				$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last td:eq("+v+")").html(repAll(resultData[1][i][v], "\n", "<br/>"));
		...
		$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last").clone().insertAfter("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last");
		for(var v = 0; v < cellsCount; v++) {
			$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:last td:eq("+v+")").html('<input title="показать на графике" id="TAB1_cb'+(v+1)+'" class="TAB1_0" type="checkbox" points="['+graphArrayOne[1][v]+']" pointsTwo="['+graphArrayTwo[1][v]+']" />');
		}
		
		$("div.drawnResults-"+time+" .TAB1_0.NSW.M_1:first").remove();
		$("div.drawnResults-"+time+" .TAB1_0").css("display", "block");
		$("div.drawnResults-"+time+" tr.TAB1_0").css("display", "table-row");
		
		tIndex = 1;
		$("div.drawnResults-"+time+" tr.TAB1_0").each(function(i,o) {
			if(i < $("div.drawnResults-"+time+" tr.TAB1_0").length-1) {
				if($(o).hasClass("NSW")) {
	...
	if(typeof resultData[2] != 'undefined') {
		$("div.drawnResults-"+time+" .TAB2_0.NSW.M_1:not(:last)").remove();
		var cellsCount = $("div.drawnResults-"+time+" .TAB2_0.NSW.M_1:last td").length;
		...
		for(var i = 0; i <= resultData[4]["NSW"]; i++) {
			$("div.drawnResults-"+time+" .TAB2_0.NSW.M_1:last").clone().insertAfter("div.drawnResults-"+time+" .TAB2_0.NSW.M_1:last");
			for(var v = 0; v < cellsCount; v++) {
				$("div.drawnResults-"+time+" .TAB2_0.NSW.M_1:last td:eq("+v+")").html(repAll(resultData[2][i][v], "\n", "<br/>"));
		...
		$("div.drawnResults-"+time+" .TAB2_0.NSW.M_"+thisM+":last").clone().insertAfter("div.drawnResults-"+time+" .TAB2_0.NSW.M_"+thisM+":last");
		for(var v = 0; v < cellsCount; v++) {
			$("div.drawnResults-"+time+" .TAB2_0.NSW.M_"+thisM+":last td:eq("+v+")").html('<input title="показать на графике" id="TAB2_cb'+(v+1)+'" class="TAB2_0" type="checkbox" points="['+graphArrayOne[2][v]+']" pointsTwo="['+graphArrayTwo[2][v]+']" />');
		}
		
		$("div.drawnResults-"+time+" .TAB2_0.NSW.M_1:first").remove();
		$("div.drawnResults-"+time+" .TAB2_0").css("display", "block");
		$("div.drawnResults-"+time+" tr.TAB2_0").css("display", "table-row");
		
		tIndex = 1;
		$("div.drawnResults-"+time+" tr.TAB2_0").each(function(i,o) {
			if(i < $("div.drawnResults-"+time+" tr.TAB2_0").length-1) {
				if($(o).hasClass("NSW")) {
					$(o).prepend("<td>"+tIndex+"</td>");
					tIndex++;
				}
	...
	if(typeof resultData[3] != 'undefined') {
		$("div.drawnResults-"+time+" .TAB3_0.NSW.M_2:not(:last)").remove();
		var cellsCount = $("div.drawnResults-"+time+" .TAB3_0.NSW.M_2:last td").length;
		...
		for(var i = 0; i <= resultData[4]["NSW"]; i++) {
			$("div.drawnResults-"+time+" .TAB3_0.NSW.M_2:last").clone().insertAfter("div.drawnResults-"+time+" .TAB3_0.NSW.M_2:last");
			for(var v = 0; v < cellsCount; v++) {
				$("div.drawnResults-"+time+" .TAB3_0.NSW.M_2:last td:eq("+v+")").html(repAll(resultData[3][i][v], "\n", "<br/>"));
		...
		$("div.drawnResults-"+time+" .TAB3_0.NSW.M_"+thisM+":last").clone().insertAfter("div.drawnResults-"+time+" .TAB3_0.NSW.M_"+thisM+":last");
		for(var v = 0; v < cellsCount; v++) {
			$("div.drawnResults-"+time+" .TAB3_0.NSW.M_"+thisM+":last td:eq("+v+")").html('<input title="показать на графике" id="TAB3_cb'+(v+1)+'" class="TAB3_0" type="checkbox" points="['+graphArrayOne[3][v]+']" pointsTwo="['+graphArrayTwo[3][v]+']" />');
		}
		
		$("div.drawnResults-"+time+" .TAB3_0.NSW.M_2:first").remove();
		$("div.drawnResults-"+time+" .TAB3_0").css("display", "block");
		$("div.drawnResults-"+time+" tr.TAB3_0").css("display", "table-row");
		
		tIndex = 1;
		$("div.drawnResults-"+time+" tr.TAB3_0").each(function(i,o) {
			if(i < $("div.drawnResults-"+time+" tr.TAB3_0").length-1) {
				if($(o).hasClass("NSW")) {
					$(o).prepend("<td>"+tIndex+"</td>");
					tIndex++;
				}
...
function createPDF(obj) {
	...
	resultsWindow = window.open("http://www.simumath.net/tools/utils/pdf.php?c="+btoa(page), 'drawnPDF', 'width=1024, height=800');
	
	return false;
}

function showGraph(tabNum, time) {
	console.log("div.drawnResults-"+time+" input.TAB"+tabNum+"_0:checked");
	loadingIcon(true);
